...
module Value = struct
  ...
  let rec expand_value = function
    | V_Neutral (H_Free_global { defn = Some defn; _ }, es) ->
  ...
  (* FIXME: equality test rather than reify to term and check for
     alpha equality? This would mean that we wouldn't need to use the
     'Irrel' constructor. *)
...
module Evaluation : sig
  ...
  let evaluate ctxt tm =
    ...
    and eval_head h env = match h.head_data with
      ...
      | Coerce { coercee; src_type; tgt_type; _ } ->
