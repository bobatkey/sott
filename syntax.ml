...
let app (x, y) =
  match x with
    | Neutral (h, es) -> Neutral (h, App (es, y))
...
let leibniz_ty =
  Pi (Set,
      Pi (bnd 0,
          Pi (Pi (bnd 1, Set),
              Pi (Neutral (Bound 0, App (Nil, bnd 1)),
                  Pi (bnd 3,
                      Pi (TmEq (bnd 3, bnd 4, bnd 0, bnd 4),
                          Neutral (Bound 3, App (Nil, bnd 1))))))))

let leibniz_tm =
  Lam (Lam (Lam (Lam (Lam (Lam (Neutral (Coerce
                                           { coercee  = bnd 2
                                           ; src_type = app (bnd 3, bnd 4)
                                           ; tgt_type = app (bnd 3, bnd 1)
                                           ; eq_proof =
                                               Subst { ty_s = bnd 5
                                                     ; ty_t = app (bnd 4, bnd 0)
                                                     ; tm_x = bnd 4
                                                     ; tm_y = bnd 1
                                                     ; tm_e = bnd 0
                                                     }}, Nil)))))))

let leibniz_tm_wrong =
  Lam (Lam (Lam (Lam (Lam (Lam (Neutral (Coerce
                                           { coercee  = bnd 2
                                           ; src_type = app (bnd 3, bnd 4)
                                           ; tgt_type = app (bnd 3, bnd 1)
...
(*
let test =
  Lam (Lam (Lam (Lam (app (app (app (app (app (app (leibniz_tm, bnd 3), bnd 2), bnd 1), bnd 0), bnd 2), Refl)))))
*)

let close' x = function Lam t -> close x 0 t | _ -> assert false
